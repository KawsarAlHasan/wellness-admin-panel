...
import { LeftOutlined, RightOutlined, ClockCircleOutlined, EnvironmentOutlined } from '@ant-design/icons';
...
function UpcomingEvent() {
  ...
  const renderCalendar = () => {
    ...
    for (let i = startingDayOfWeek - 1; i >= 0; i--) {
      days.push(
        <div key={`prev-${i}`} className="text-center py-3 text-gray-300 text-sm">
          {prevMonthDays - i}
        </div>
      );
    }
        ...
        {Object.entries(groupedEvents).map(([date, events]) => (
          <div key={date}>
            <h3 className="text-lg font-semibold mb-4">
              {date.split(',')[0]} <span className="font-normal">{date.split(' ').slice(1).join(' ')}</span>
            </h3>
              ...
              {events.map((event) => (
                <div key={event.id} className="flex gap-4 pb-4 border-b border-gray-100 last:border-0">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mb-3">{event.clubName}</p>
